...
import { StickyNote, Plus, Trash2, CheckCircle2, CornerDownLeft, Pencil, Check } from "lucide-react"
...
function NoteItem({ 
  ...
  return (
    ...
    >
      <Card className={cn(
        "flex flex-row items-start gap-5 py-5 px-6 transition-all duration-500 border-white/5 shadow-sm relative overflow-hidden ",
        note.checked 
          ...
          >
            <AnimatePresence>
              {note.checked && (
                ...
                >
                  <CheckCircle2 className="w-4 h-4 text-primary-foreground stroke-[3]" />
                </motion.div>
              )}
            </AnimatePresence>
          </div>
          ...
          {isEditing ? (
            <div className="flex-1 space-y-2">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <Button size="sm" variant="ghost" onClick={() => { setEditedText(note.text); setIsEditing(false); }}>Cancel</Button>
            ...
            <div className="flex-1 flex items-center gap-2 group/text">
              ...
              <Button
                ...
                size="icon"
                onClick={() => setIsEditing(true)}
                className="h-7 w-7 opacity-0 group-hover/text:opacity-100 transition-opacity mt-0.5 shrink-0"
              >
        ...
        <AlertDialog>
          <AlertDialogTrigger asChild>
            <Button
              variant="ghost"
              size="icon"
              className="h-9 w-9 rounded-xl text-muted-foreground/40 hover:text-destructive hover:bg-destructive/10 transition-all z-20 shrink-0"
              title="Delete note"
            >
          ...
          <AlertDialogContent className="bg-card border-white/5 backdrop-blur-xl">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription className="text-muted-foreground">
                Are you sure you want to remove this note? This cannot be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
...
export function NotesSection() {
  ...
  return (
    <div className="space-y-8">
      <div className="relative group/input bg-card/40 rounded-2xl border border-white/5 p-2 focus-within:border-primary/40 transition-all duration-500 shadow-inner">
        <Textarea
          value={newNoteText}
        ...
        <div className="flex justify-between items-center px-2 py-2 border-t border-white/5">
          <span className="text-[10px] font-bold text-muted-foreground/50 uppercase tracking-widest ml-2 flex items-center gap-1">
           ...
           <Button
            onClick={() => handleAdd()} 
            type="submit" size="sm" className="rounded-lg shadow-sm shadow-primary/10 cursor-pointer">
      ...
      {notes.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-center space-y-6">
          ...
          <div className="space-y-2">
            <h3 className="text-xl font-black text-foreground/80 tracking-tight">No notes yet</h3>
